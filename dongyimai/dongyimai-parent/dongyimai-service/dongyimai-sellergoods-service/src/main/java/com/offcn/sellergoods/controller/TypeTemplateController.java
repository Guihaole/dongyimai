...
public class TypeTemplateController {
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Long")
    @GetMapping("/{id}")
    public Result<List<Map>> findById(@PathVariable Long id){
        //调用TypeTemplateService实现根据主键查询TypeTemplate
        Result<List<Map>> result = typeTemplateService.findById(id);
        return result;
    }
    ...
    /**
     * 需求： 根据type_id 查询spec_ids 分类id,返回某个分类下的规格选项名option_name
     * @return
     */
